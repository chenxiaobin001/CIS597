...
RSpec.configure do |config|
  config.before(:each) do
    f = open("testCases/countries.txt")
    doc = f.read()
    f.close()

=begin   
    af_f = open("testCases/af.txt")
    af_doc = af_f.read()
    ax_f = open("testCases/ax.txt")
    ax_doc = ax_f.read()
=end
    stub_request(:get, %Q|https://www.cia.gov/library/publications/the-world-factbook/print/textversion.html|).
      with(headers: {'Accept'=>'*/*', 'User-Agent'=>'Ruby'}).
      to_return(status: 200, body: doc, headers: {})

    stub_request(:get, /countrytemplate_/).
      with(headers: {'Accept'=>'*/*', 'User-Agent'=>'Ruby'}).
      to_return(status: 200, body: "empty", headers: {})

    stub_request(:get, %Q|https://www.cia.gov/library/publications/the-world-factbook/print/../geos/countrytemplate_af.html|).
      with(headers: {'Accept'=>'*/*', 'User-Agent'=>'Ruby'}).
      to_return(:body => open('testCases/af.txt'), :status => 200)
=begin    
    stub_request(:get, %Q|https://www.cia.gov/library/publications/the-world-factbook/geos/countrytemplate_ax.html|).
      with(headers: {'Accept'=>'*/*', 'User-Agent'=>'Ruby'}).
      to_return(status: 200, body: ax_doc, headers: {})
=end
...
describe "helloWorld spec" do
  ...
  puts "start testing: mock test (prepared test_cases/request_response_contents are in folder 'testCases'\n make sure this folder is present before running test!)"
